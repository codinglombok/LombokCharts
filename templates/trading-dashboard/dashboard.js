  ...
  function boot() {
    ...
    class VolBarsMark extends Mark {
      ...
      draw(ctx) {
        ...
        const step =
          rawData.length > 1
            ? Math.abs(sx(rawData[1].x) - sx(rawData[0].x))
            : 10;
        ...
        for (let i = 0; i < rawData.length; i++) {
          const d = rawData[i],
            cx = sx(d.x),
            yv = sy(d.volume);
          r.rect(cx - w / 2, yv, w, Math.max(0, y0 - yv), {
            fill: d.up ? GREEN : RED,
            opacity: 0.5,
          });
          ctx.hits.push({
    ...
    function buildCharts() {
      ...
      panels.forEach((p) =>
        p.on("crosshair", (info) => {
          panels.forEach((q) => {
            if (q !== p) q.setCrosshair(info ? info.x : null);
          });
          if (info) {
            const i = Math.round(info.x);
            const c = market.candles[i];
            if (c) updateReadout(c);
          } else clearReadout();
        }),
      );
    }
    ...
    function updateReadout(c) {
      ...
      el.innerHTML =
        `O ${fmtP(c.open)}  H ${fmtP(c.high)}  L ${fmtP(c.low)}  C <span style="color:${up ? GREEN : RED}">${fmtP(c.close)}</span>  ` +
    ...
    function renderBook() {
      ...
      const row = (o, side) => {
        ...
        return (
          `<div class="ob-row ${side}"><span class="ob-depth" style="width:${w}%"></span>` +
      ...
      $("spread").textContent =
        `Spread ${fmtP(book.spread)} (${((book.spread / market.last().close) * 100).toFixed(3)}%)`;
    }
    ...
    function refreshHeader() {
      ...
      badge.className =
        "badge badge-pill " + (chg >= 0 ? "badge-success" : "badge-danger");
      let hi = -Infinity,
        lo = Infinity,
        vol = 0;
      for (let i = Math.max(0, c.length - COUNT); i < c.length; i++) {
        hi = Math.max(hi, c[i].high);
        lo = Math.min(lo, c[i].low);
        vol += c[i].volume;
      }
      $("high").textContent = fmtP(hi);
      $("low").textContent = fmtP(lo);
      $("vol24").textContent = fmtV(vol);
    }
    ...
    function buildWatchlist() {
      ...
      sparks = {};
      ticker = M.createTicker(
        Object.keys(SYMBOLS).map((s) => ({
          symbol: s,
          price: SYMBOLS[s].start,
          vol: SYMBOLS[s].vol,
        })),
        48,
      );
      $("watchlist").innerHTML = ticker.assets
        .map(
          (a) =>
            `<div class="wl-row"><span class="wl-sym">${a.symbol}</span>` +
            `<span class="wl-px" id="wl_${a.symbol.replace(/\W/g, "")}">${fmtP(a.price)}</span>` +
            `<div class="spark" id="sp_${a.symbol.replace(/\W/g, "")}"></div></div>`,
        )
        .join("");
      ticker.assets.forEach((a) => {
        const id = a.symbol.replace(/\W/g, "");
        sparks[a.symbol] = chart("#sp_" + id, {
          xs: Float64Array.from(a.spark.map((_, i) => i)),
          ys: Float64Array.from(a.spark),
          mark: "line",
          theme: { name: "dark", colors: { background: "transparent" } },
          animate: false,
          axes: false,
          grid: false,
          tooltip: false,
    ...
    function refreshWatchlist() {
      ...
      ticker.assets.forEach((a) => {
        ...
        const px = $("wl_" + id);
        if (px) {
          px.textContent = fmtP(a.price);
          px.style.color = a.changePct >= 0 ? GREEN : RED;
        }
        sparks[a.symbol].update({
          xs: Float64Array.from(a.spark.map((_, i) => i)),
          ys: Float64Array.from(a.spark),
          count: a.spark.length,
        });
      });
    }
    ...
    function setSide(s) {
      side = s;
      $("buyBtn").className =
        "btn btn-sm " + (s === "buy" ? "btn-primary" : "btn-soft");
      $("sellBtn").className =
        "btn btn-sm " + (s === "sell" ? "btn-danger" : "btn-soft");
      $("place").className =
        "btn btn-block " + (s === "buy" ? "btn-primary" : "btn-danger");
      $("place").textContent =
        (s === "buy" ? "Buy " : "Sell ") + symbol.split("-")[0];
    }
    function currentPrice() {
      return parseFloat($("price").value) || market.last().close;
    }
    ...
    function placeOrder() {
      ...
      tr.className = "tr";
      tr.innerHTML =
        `<span class="badge ${side === "buy" ? "badge-success" : "badge-danger"} badge-pill">${side.toUpperCase()}</span>` +
