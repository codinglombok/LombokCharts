...
export class PointMark extends Mark {
  domains(series, opts) {
    ...
    return {
      x: { type: opts.xScale || "linear", domain: [xmin - px, xmax + px] },
      y: { domain: [ymin - py, ymax + py] },
    };
  }
  ...
  draw(ctx) {
    ...
    series.forEach((s, si) => {
      ...
      if (mode === "bubble" && s.sizes) {
        ...
        const rscale = sqrtScale(
          [smin, smax],
          [opts.minRadius || 4, opts.maxRadius || 28],
        );
        for (let i = 0; i < s.count; i++) {
          const px = sx(s.xs[i]),
            py = sy(s.ys[i]);
          r.circle(px, py, rscale(s.sizes[i]), {
            fill: color,
            opacity: 0.6,
            stroke: color,
          });
        ...
        r.pointsTyped(s.xs, s.ys, s.count, sx, sy, rad, {
          fill: color,
          opacity: s.count > 5000 ? 0.5 : 0.8,
        });
        drawn += s.count;
        if (s.count <= 4000)
          for (let i = 0; i < s.count; i++)
