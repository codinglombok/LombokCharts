...
export class BarMark extends Mark {
  domains(series, opts) {
    ...
    if (mode === 'stacked') {
      ...
      for (let i = 0; i < n; i++) {
        let pos = 0, neg = 0;
        for (const s of series) { const v = s.ys[i]; if (v >= 0) pos += v; else neg += v; }
        if (pos > ymax) ymax = pos; if (neg < ymin) ymin = neg;
      }
    } else if (mode === 'waterfall') {
      let acc = 0;
      const s = series[0];
      for (let i = 0; i < s.count; i++) { acc += s.ys[i]; if (acc > ymax) ymax = acc; if (acc < ymin) ymin = acc; }
    } else {
      for (const s of series) for (let i = 0; i < s.count; i++) { const v = s.ys[i]; if (v > ymax) ymax = v; if (v < ymin) ymin = v; }
    }
    if (ymax < 0) ymax = 0;
  ...
  draw(ctx) {
    ...
    if (mode === 'stacked') {
      ...
      for (let i = 0; i < s.count; i++) {
        const v = s.ys[i];
        const start = acc; const end = acc + v;
        const bx = sx(cats[i]);
