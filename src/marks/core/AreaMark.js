...
export class AreaMark extends Mark {
  domains(series, opts) {
    ...
    if (mode === 'stacked' || mode === 'streamgraph') {
      ...
      for (let i = 0; i < n; i++) {
        let tot = 0; for (const s of series) tot += s.ys[i];
  ...
  draw(ctx) {
    ...
    if (mode === 'stacked' || mode === 'streamgraph') {
      ...
      if (mode === 'streamgraph') for (let i = 0; i < n; i++) { let t = 0; for (const s of series) t += s.ys[i]; baseArr[i] = -t / 2; }
      const visible = series.filter((s) => s.visible !== false);
      visible.forEach((s, si) => {
        const top = []; const bottom = [];
        for (let i = 0; i < n; i++) {
          const px = xPix(s.xs[i], i);
          const b = baseArr[i]; const t = b + s.ys[i];
          top.push(px, sy(t)); bottom.unshift(px, sy(b));
          baseArr[i] = t;
        }
      ...
      series.forEach((s, si) => {
        ...
        if (opts.decimate !== false && count > target && sx.kind !== 'band') { const d = lttb(xs, ys, count, target); xs = d.xs; ys = d.ys; count = d.count; }
        ...
        drawn += count;
        if (count <= 2000) for (let i = 0; i < count; i++) ctx.hits.push({ x: xPix(xs[i], i), y: sy(ys[i]), seriesIndex: si, index: i, label: s.label, value: ys[i], color });
      });
    }
    ctx.stats = { drawn };
  }
