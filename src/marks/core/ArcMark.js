...
export class ArcMark extends Mark {
  ...
  draw(ctx) {
    ...
    if (mode === "gauge") {
      ...
      r.arcSlice(cx, cy, rIn, maxR, a0, a0 + (a1 - a0) * t, {
        fill: s.color || ctx.color.byIndex(0),
      });
      r.text(cx, cy, String(value), {
      ...
      ctx.hits.push({
        ...
        label: "value",
        value,
        color: s.color || ctx.color.byIndex(0),
      });
      ctx.stats = { drawn: 2 };
      return;
    }
    ...
    if (mode === "radial-bar") {
      ...
      vals.forEach((v, i) => {
        ...
        r.arcSlice(
          ...
          rr,
          -Math.PI / 2,
          -Math.PI / 2 + (Math.PI * 2 * v) / maxV,
          { fill: color },
        );
        r.arcSlice(cx, cy, rr - ring * 0.8, rr, -Math.PI / 2, Math.PI * 1.5, {
    ...
    vals.forEach((v, i) => {
      ...
      const color =
        s.color && Array.isArray(s.color) ? s.color[i] : ctx.color.byIndex(i);
      r.arcSlice(cx, cy, rIn, maxR, angle, angle + sweep, {
        fill: color,
      ...
      if (sweep > 0.35)
        r.text(lp.x, lp.y, Math.round((v / total) * 100) + "%", {
