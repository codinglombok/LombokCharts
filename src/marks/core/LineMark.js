...
export class LineMark extends Mark {
  domains(series, opts) {
    ...
    return {
      ...
      y: {
        domain: [
          opts.baseline === 0 ? Math.min(0, ymin) : ymin - pad,
          ymax + pad,
        ],
      },
    };
  ...
  draw(ctx) {
    ...
    const xPix =
      sx.kind === "band"
        ? (v, i) => sx.center(series[0].categories[i])
        : (v) => sx(v);
        ...
        r.polylineTyped(
          ...
          (v) =>
            sx.kind === "band"
              ...
              : sx(v),
          sy,
          { stroke: color, width: opts.width || 2 },
        );
      }
      ...
      if (count <= 2000) {
        for (let i = 0; i < count; i++) {
          const px = xPix(xs[i], i),
            py = sy(ys[i]);
          if (opts.points)
            r.circle(px, py, opts.pointRadius || 3, { fill: color });
          ctx.hits.push({
            x: px,
            y: py,
            seriesIndex: si,
...
function catmullRom(xs, ys, n, fx, fy) {
  ...
  for (let i = 0; i < n - 1; i++) {
    const x0 = fx(xs[Math.max(0, i - 1)], Math.max(0, i - 1)),
      y0 = fy(ys[Math.max(0, i - 1)]);
    const x1 = fx(xs[i], i),
      y1 = fy(ys[i]);
    const x2 = fx(xs[i + 1], i + 1),
      y2 = fy(ys[i + 1]);
    const x3 = fx(xs[Math.min(n - 1, i + 2)], Math.min(n - 1, i + 2)),
      y3 = fy(ys[Math.min(n - 1, i + 2)]);
    cmds.push([
      "C",
      x1 + (x2 - x0) / 6,
      y1 + (y2 - y0) / 6,
