...
export class HeatmapMark extends Mark {
  ...
  draw(ctx) {
    ...
    for (const d of data) { const v = +d[vk]; if (v < vmin) vmin = v; if (v > vmax) vmax = v; }
    const pad = { l: 70, b: 28 };
    ...
    for (const d of data) {
      ...
      ctx.hits.push({ x: x + gx.bandwidth / 2, y: y + gy.bandwidth / 2, seriesIndex: 0, index: 0, label: `${d[yk]} · ${d[xk]}`, value: v, color: cscale(v) });
    }
