...
export class CandlestickMark extends Mark {
  _rows(ctx) {
    ...
    return data.map((d, i) => ({
      ...
      open: +d[o.open || 'open'], high: +d[o.high || 'high'], low: +d[o.low || 'low'], close: +d[o.close || 'close'],
    }));
  }
  domains(series, opts, rawData) {
    const rows = this._rows({ rawData });
    let lo = Infinity, hi = -Infinity, xmin = Infinity, xmax = -Infinity;
    for (const r of rows) { lo = Math.min(lo, r.low); hi = Math.max(hi, r.high); xmin = Math.min(xmin, r.x); xmax = Math.max(xmax, r.x); }
  ...
  draw(ctx) {
    ...
    for (let i = 0; i < rows.length; i++) {
      const d = rows[i]; const cx = sx(d.x);
