...
export class RadarMark extends Mark {
  ...
  draw(ctx) {
    ...
    const n = axes.length;
    let max = -Infinity;
    for (const s of series) { const [, b] = extentTyped(s.ys, s.count); if (b > max) max = b; }
    max = opts.max || max || 1;
    ...
    for (let g = 1; g <= rings; g++) {
      ...
      const pts = [];
      for (let i = 0; i <= n; i++) { const p = polarToCartesian(cx, cy, rr, ang(i % n)); pts.push(p.x, p.y); }
    ...
    series.forEach((s, si) => {
      ...
      const pts = [];
      for (let i = 0; i < n; i++) { const rr = (s.ys[i] / max) * maxR; const p = polarToCartesian(cx, cy, rr, ang(i)); pts.push(p.x, p.y); ctx.hits.push({ x: p.x, y: p.y, seriesIndex: si, index: i, label: axes[i], value: s.ys[i], color }); }
      r.polyline(pts, { fill: hexA(color, 0.18), stroke: color, width: 2, closed: true });
    });
    ctx.stats = { drawn: series.length };
  }
  legendItems(series, ctx) { return series.length > 1 ? series.map((s, i) => ({ label: s.label || 'Series ' + (i + 1), color: s.color || ctx.color.byIndex(i) })) : null; }
}
