...
export class BoxPlotMark extends Mark {
  _groups(ctx) {
    if (this.options.groups)
      return this.options.groups.map((g) => ({
        label: g.label,
        values: [...g.values].sort((a, b) => a - b),
      }));
    const data = ctx.rawData || [];
    const gk = this.options.group || "group",
      vk = this.options.value || "value";
    const map = new Map();
    for (const d of data) {
      const k = String(d[gk]);
  ...
  domains(series, opts, rawData) {
    ...
    return {
      x: { type: "band", values: groups.map((g) => g.label) },
  ...
  draw(ctx) {
    ...
    groups.forEach((g, i) => {
      ...
      const q1 = quantileSorted(v, 0.25),
        q2 = quantileSorted(v, 0.5),
        q3 = quantileSorted(v, 0.75);
      const iqr = q3 - q1;
      const lo = Math.max(v[0], q1 - 1.5 * iqr),
        hi = Math.min(v[v.length - 1], q3 + 1.5 * iqr);
      ...
      r.line(cx - bw / 4, sy(hi), cx + bw / 4, sy(hi), {
        stroke: color,
        width: 1,
      });
      r.line(cx - bw / 4, sy(lo), cx + bw / 4, sy(lo), {
        stroke: color,
        width: 1,
      });
      r.rect(cx - bw / 2, sy(q3), bw, sy(q1) - sy(q3), {
        fill: hexA(color, 0.3),
        stroke: color,
        width: 1.5,
        radius: 2,
      });
      r.line(cx - bw / 2, sy(q2), cx + bw / 2, sy(q2), {
        stroke: color,
        width: 2,
      });
      for (const pt of v)
        if (pt < lo || pt > hi)
          r.circle(cx, sy(pt), 2, { fill: theme.semantic.negative });
      ctx.hits.push({
        x: cx,
        y: sy(q2),
        seriesIndex: 0,
        ...
        value: q2,
        color,
        extra: `Q1 ${q1.toFixed(1)} · Q3 ${q3.toFixed(1)}`,
      });
    });
