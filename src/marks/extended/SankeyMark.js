...
export class SankeyMark extends Mark {
  ...
  draw(ctx) {
    ...
    links.forEach((l) => { const t = byId.get(l.target); if (t) t.layer = Math.max(t.layer, (byId.get(l.source)?.layer ?? 0) + 1); });
    const maxLayer = Math.max(0, ...nodes.map((n) => n.layer));
    nodes.forEach((n) => { n.out = links.filter((l) => l.source === (n.id ?? n.name)).reduce((s, l) => s + l.value, 0); n.in = links.filter((l) => l.target === (n.id ?? n.name)).reduce((s, l) => s + l.value, 0); n.total = Math.max(n.in, n.out); });
    const colW = 16;
    ...
    layers.forEach((ns, li) => {
      let y = area.y;
      ns.forEach((n) => { n.x = layerX(li); n.y = y; n.h = n.total * scale; n.cursorOut = n.y; n.cursorIn = n.y; y += n.h + 12; });
    });
    // links
    links.forEach((l) => {
      ...
      const sy0 = s.cursorOut; s.cursorOut += sh;
      const ty0 = t.cursorIn; t.cursorIn += th;
      const x0 = s.x + colW, x1 = t.x;
      const xm = (x0 + x1) / 2;
      const color = ctx.color.byIndex(s.i);
      r.pathCmds([
        ['M', x0, sy0], ['C', xm, sy0, xm, ty0, x1, ty0],
        ['L', x1, ty0 + th], ['C', xm, ty0 + th, xm, sy0 + sh, x0, sy0 + sh], ['Z'],
