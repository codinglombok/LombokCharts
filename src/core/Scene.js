  ...
  drawAxes(r, area, scales, opts = {}) {
    ...
    if (sy && sy.ticks) {
      ...
      for (const v of yticks) {
        ...
        if (!yIsBand && opts.showGrid !== false)
          r.line(area.x, py, area.x + area.width, py, {
    ...
    if (sx && sx.ticks) {
      ...
      xticks.forEach((v, i) => {
        ...
        if (!isBand && opts.showGrid === "both")
          r.line(px, area.y, px, bottom, { stroke: t.colors.grid, width: 1 });
        r.line(px, bottom, px, bottom + t.spacing.tickLength, {
          ...
          typeof v !== "number"
            ...
            : opts.xTickFormat
              ...
              : sx.tickFormat
                ? sx.tickFormat(v)
                : formatNumber(v);
        r.text(px, bottom + t.spacing.tickLength + 3, label, {
          ...fontAxis,
          align: "center",
          baseline: "top",
        });
    ...
    r.line(area.x, bottom, area.x + area.width, bottom, {
      stroke: t.colors.axis,
      width: 1,
    });

    if (opts.yLabel)
      r.text(12, area.y + area.height / 2, opts.yLabel, {
        ...fontAxis,
        fill: t.colors.text,
        align: "center",
        baseline: "middle",
    ...
    if (opts.xLabel)
      r.text(area.x + area.width / 2, bottom + 28, opts.xLabel, {
        ...fontAxis,
        fill: t.colors.text,
        align: "center",
        baseline: "top",
